...
function MatriculaDashboard() {
  ...
  useEffect(() => {
    apiGet<any[]>("/planificacion/paralelos/").then(setParalelos).catch(() => {
      // Si falla, usamos paralelos de demo para el KPI
      setParalelos([{ id: 1, cupos_maximo: 35, cupos_ocupados: 10 }]);
    });
  }, []);
  ...
  const cuposDisponibles = paralelos.reduce((acc, p) => acc + ((p.cupos_maximo || 0) - (p.cupos_ocupados || 0)), 0);

  const matriculasFiltradas = matriculas.filter(m => !filtroEstado || m.estado === filtroEstado);

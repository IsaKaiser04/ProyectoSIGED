...
import { apiPost, buildModulePath } from '../../../services/apiClient';
...
export const FormularioPais: React.FC<Props> = ({ onSaveSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setEnviando(true);
      await apiPost(buildModulePath('ubicacion', 'paises'), { nombre });
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background: 'var(--surface-container-lowest)', borderRadius: '10px', overflow: 'hidden' }}>
      <div style={{ padding: '20px', borderBottom: '1px solid var(--outline-variant)' }}>
        <h2 style={{ margin: 0, color: 'var(--primary)', fontSize: '22px', fontWeight: '700' }}>Registrar País</h2>
        <p style={{ marginTop: '6px', color: 'var(--on-surface-variant)', fontSize: '14px' }}>Ingrese el nombre del nuevo país.</p>
      </div>
      <div style={{ padding: '20px' }}>
        <div>
          <label style={labelStyle}>Nombre del País</label>
          <input style={fieldStyle} value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej. Ecuador" required />
        </div>
      </div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', padding: '20px', borderTop: '1px solid var(--outline-variant)', background: 'var(--surface-container-low)' }}>
        ...
        <button type="submit" disabled={enviando} style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: 'var(--secondary)', color: 'white', fontWeight: '600', cursor: 'pointer' }}>
          {enviando ? 'Guardando...' : 'Guardar País'}
        </button>
      </div>
    </form>
  );

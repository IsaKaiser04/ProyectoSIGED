...
import { DialogCard, GlassInfoCard, CARD_COLORS, percent } from "../../components/DashboardCards";
import { useAuth } from "../../features/autenticacion/context/AuthContext";

export function DocenteInicioDashboard() {
  const { usuario } = useAuth();
  const userNombres = usuario?.datos_personales?.nombres ?? "";
  const userApellidos = usuario?.datos_personales?.apellidos ?? "";
  const userNombreCompleto = `${userNombres} ${userApellidos}`.trim() || "Juan Carlos Mendoza López";
  const userIdentificacion = usuario?.datos_personales?.identificacion || "0999999999";
  const userCorreo = usuario?.correo_institucional || "juan.mendoza@inst.edu";
      ...
      <div style={{ padding: 24 }}>
        ...
        >
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            >
              <GlassInfoCard icon={Hash} label="Identificación" value={userIdentificacion} iconBg="#eff6ff" iconBorder="#93c5fd" iconColor="#1d4ed8" />
              ...
              <GlassInfoCard icon={Mail} label="Correo" value={userCorreo} iconBg="#f5f3ff" iconBorder="#c4b5fd" iconColor="#6d28d9" />

// frontend/src/features/planificacion/components/TabPlanEstudio.tsx
import React from 'react';
import { PlanEstudio, Grado, Asignatura, EducacionNivel, EducacionSubNivel } from '../../../types/entities/planificacion';;

interface TabPlanEstudioProps {
  planes: PlanEstudio[];
  grados: Grado[];
...
export const TabPlanEstudio: React.FC<TabPlanEstudioProps> = ({
  planes, grados, asignaturas, niveles, subNiveles, planForm
}) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', width: '100%' }}>
      
      {/* ========== PLANES DE ESTUDIO ========== */}
      <form onSubmit={planForm.handleAgregarPlan} style={{ 
        ...
        <div style={{ flex: 1 }}>
          ...
          <input
            ...
            value={planForm.nuevoPlan}
            onChange={(e) => planForm.setNuevoPlan(e.target.value)}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', paddingBottom: '8px' }}>
          <input
            type="checkbox"
            id="esActivoPlan"
            checked={planForm.esActivoPlan}
            onChange={(e) => planForm.setEsActivoPlan(e.target.checked)}
              ...
              planes.map((plan, index) => (
                <tr key={plan.id} style={{ borderBottom: '1px solid var(--outline-variant)' }}>
                  ...
                  <td style={{ padding: '12px 20px', textAlign: 'center' }}>
                    ...
                    <button onClick={() => planForm.handleEliminarPlan(plan.id)} style={{ background: 'transparent', border: 'none', cursor: 'pointer' }}>🗑️</button>
                  </td>
                </tr>
              ))
            )}

...
class EstudianteListCreateView(InstitucionFilterMixin, generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if anio_lectivo_id or paralelo_id:
            ...
            matriculas = Matricula.objects.filter(filters).values_list('estudiante_id', flat=True).distinct()
...
class ListaEstudiantesRegistroNotasView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        paralelo_id = request.GET.get('paralelo_id') or request.GET.get('curso_id')

        resultados = []

        # Si no hay filtros, devolver todos los estudiantes con perfil
        if not anio_lectivo_id and not paralelo_id:
        ...
        if anio_id and paralelo_id_val:
            filters = Q(anio_lectivo_id=anio_id) & Q(paralelo_id=paralelo_id_val)
            ids_matriculados = set()
            for m in Matricula.objects.filter(filters):
                if m.estudiante_id:
                    ids_matriculados.add(m.estudiante_id)

            # Agregar estudiantes encontrados
            if ids_matriculados:
                for e in Estudiante.objects.filter(id__in=ids_matriculados):
                    resultados.append({
                        'id': e.id,
                        'nombres': e.nombres,
                        'apellidos': e.apellidos,
